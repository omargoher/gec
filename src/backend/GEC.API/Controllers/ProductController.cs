...
public class ProductController : ControllerBase
{
    ...
    public async Task<ActionResult<VariantResponse>> ReassignAttributeValueAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        // For RESTful completeness, enforce path param matches request payload
        if (attributeId != request.AttributeId)
            return BadRequest(new ProblemDetails { Detail = "AttributeId in path does not match body." });
